...
from emergentintegrations.llm.gemeni.video_generation import GeminiVideoGeneration
...
from config import (
    ...
    VIDEO_MODELS, VIDEO_MODEL_LABELS, VIDEO_MODEL_PROVIDER,
    VIDEO_SIZES, VIDEO_SIZE_LABELS, VIDEO_DURATIONS,
    IMAGE_MODELS, IMAGE_MODEL_LABELS, IMAGE_MODEL_PROVIDER,
    IMAGE_QUALITY,
    TTS_MODELS, TTS_VOICES, TTS_VOICES_EXTENDED, get_tts_voices,
    MAX_RETRIES,
...
def make_video_settings_keyboard(user_id):
    ...
    rows = []
    # Model selection — one per row for clarity
    for m in VIDEO_MODELS:
        label = VIDEO_MODEL_LABELS.get(m, m)
        marker = ">> " if s["video_model"] == m else "   "
        rows.append([InlineKeyboardButton(
            text=f"{marker}{label}",
            callback_data=f"ai:vmodel:{m}",
        )])
    # Size & Duration only for OpenAI Sora models
    provider = VIDEO_MODEL_PROVIDER.get(s["video_model"], "openai")
    if provider == "openai":
...
def make_image_settings_keyboard(user_id):
    ...
    for m in IMAGE_MODELS:
        label = IMAGE_MODEL_LABELS.get(m, m)
        marker = ">> " if s["image_model"] == m else "   "
        rows.append([InlineKeyboardButton(
            text=f"{marker}{label}",
            callback_data=f"ai:imgmodel:{m}",
        )])
    # Quality (only for OpenAI models)
    provider = IMAGE_MODEL_PROVIDER.get(s["image_model"], "openai")
    if provider == "openai":
        rows.append([InlineKeyboardButton(
...
def make_tts_settings_keyboard(user_id):
    ...
    voices = get_tts_voices(s["tts_model"])
    voice_btns = [InlineKeyboardButton(
        text=(">> " if s["tts_voice"] == v else "   ") + v,
        callback_data=f"ai:ttsvoice:{v}",
    ) for v in voices]
    # Split into rows of 3
    for i in range(0, len(voice_btns), 3):
        rows.append(voice_btns[i:i+3])
    rows.append([
        InlineKeyboardButton(text="Назад", callback_data="ai:settings_hub"),
        InlineKeyboardButton(text="Главное меню", callback_data="global:main"),
    ])
...
def full_status_text(user_id):
    ...
    img_label = IMAGE_MODEL_LABELS.get(s["image_model"], s["image_model"])
    vid_label = VIDEO_MODEL_LABELS.get(s["video_model"], s["video_model"])
    vid_provider = VIDEO_MODEL_PROVIDER.get(s["video_model"], "openai")
    vid_extra = f" {s['video_size']} {s['video_duration']}сек" if vid_provider == "openai" else ""
    return (
        f"<b>Текущая конфигурация</b>\n\n"
        f"<b>Чат:</b> <code>{chat_label}</code>\n"
        f"<b>Изображения:</b> <code>{img_label}</code> ({s['image_quality']})\n"
        f"<b>Видео:</b> <code>{vid_label}</code>{vid_extra}\n"
...
def generate_video_sync(prompt, model, size, duration, output_path, api_key, ref_image_path=None):
    provider = VIDEO_MODEL_PROVIDER.get(model, "openai")
    last_error = None
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            if provider == "gemini":
                video_gen = GeminiVideoGeneration(api_key=api_key)
                kwargs = {"prompt": prompt, "max_wait_time": 900}
...
async def _do_image(message, user_id, prompt):
    ...
    t0 = time.time()
    model = s["image_model"]
    provider = IMAGE_MODEL_PROVIDER.get(model, "openai")
    model_label = IMAGE_MODEL_LABELS.get(model, model)
    ...
    try:
        if provider == "gemini":
            img_gen = GeminiImageGeneration(api_key=api_key)
            images = await img_gen.generate_images(prompt=prompt, model=model, number_of_images=s.get("image_count", 1))
        else:
            img_gen = OpenAIImageGeneration(api_key=api_key)
            images = await img_gen.generate_images(prompt=prompt, model=model, number_of_images=s.get("image_count", 1), quality=s["image_quality"])
        elapsed = time.time() - t0
        ...
        for i, img_bytes in enumerate(images):
            ...
            caption = (
                status_box("\u2714 Изображение готово", [
                    f"Модель: <code>{model_label}</code> | Качество: <code>{s['image_quality']}</code>",
...
async def cb_vmodel(call: CallbackQuery):
    ...
    if model in VIDEO_MODELS:
        ...
        s = get_settings(call.from_user.id)
        label = VIDEO_MODEL_LABELS.get(model, model)
        provider = VIDEO_MODEL_PROVIDER.get(model, "openai")
        if provider == "openai":
            text = f"<b>Настройки видео</b>\nМодель: <code>{label}</code> | Размер: <code>{s['video_size']}</code> | <code>{s['video_duration']} сек</code>"
        else:
            text = f"<b>Настройки видео</b>\nМодель: <code>{label}</code>"
        await call.message.edit_text(text, parse_mode="HTML", reply_markup=make_video_settings_keyboard(call.from_user.id))
...
@ai_router.callback_query(F.data.startswith("ai:imgmodel:"))
async def cb_imgmodel(call: CallbackQuery):
    m = call.data.split(":")[-1]
    if m in IMAGE_MODELS:
        get_settings(call.from_user.id)["image_model"] = m
        label = IMAGE_MODEL_LABELS.get(m, m)
        s = get_settings(call.from_user.id)
        await call.message.edit_text(
            f"<b>Настройки изображений</b>\nМодель: <code>{label}</code> | Качество: <code>{s['image_quality']}</code>",
            parse_mode="HTML", reply_markup=make_image_settings_keyboard(call.from_user.id))
        await call.answer(f"Модель: {label}")
...
async def cb_ttsmodel(call: CallbackQuery):
    ...
    if m in TTS_MODELS:
        s = get_settings(call.from_user.id)
        s["tts_model"] = m
        # Reset voice if current voice not available in new model
        available = get_tts_voices(m)
        if s.get("tts_voice") not in available:
            s["tts_voice"] = available[0]
        await call.message.edit_text(
            f"<b>Настройки TTS</b>\nМодель: <code>{s['tts_model']}</code> | Голос: <code>{s['tts_voice']}</code>",
            parse_mode="HTML", reply_markup=make_tts_settings_keyboard(call.from_user.id))
...
async def cb_ttsvoice(call: CallbackQuery):
    v = call.data.split(":")[-1]
    s = get_settings(call.from_user.id)
    available = get_tts_voices(s.get("tts_model", "tts-1"))
    if v in available:
        s["tts_voice"] = v
